...
struct external_potential {
  ...
  *  scale raidus times the timestep_mult */
  ...
  /*! Perturbation position (given relative to Disk center in params.yml) */
  double pert_x[3];
...
__attribute__((always_inline)) INLINE static float external_gravity_timestep(
  ...
  const float pert_dx = g->x[0] - potential->pert_x[0];
  const float pert_dy = g->x[1] - potential->pert_x[1];
  const float pert_dz = g->x[2] - potential->pert_x[2];
...
static INLINE void potential_init_backend(
  ...
  /* Read in the relative position of the perturber and convert to absolute position */
  double pertpos[3];
  parser_get_param_double_array(parameter_file, "MestelPotentialPert:pert_position",
                                3, pertpos);
  potential->pert_x[0] = pertpos[0] + potential->x[0];
  potential->pert_x[1] = pertpos[1] + potential->x[1];
  potential->pert_x[2] = pertpos[2] + potential->x[2];
...
static INLINE void potential_print_backend(
  ...
  message(
      ...
      "%e), v0 = %e, r0 = %e timestep multiplier = %e, Perturber: (x,y,z) = (%e,%e,%e)"
      "mass = %e, softening = %e",
